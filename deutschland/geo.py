        ...
        for feature in data.keys():
            for i in data[feature]["features"]:
                if i["geometry"]["type"] == "Point":
                    coords = self.__vectorToCoords(int(x),int(y) +1,self.LEVEL,i["geometry"]["coordinates"][0], i["geometry"]["coordinates"][1])
                    ...
                    for e_num, e_obj in enumerate(i["geometry"]["coordinates"]):
                        for c_num, c_obj in enumerate(e_obj):
                            coords = self.__vectorToCoords(int(x), int(y) + 1, self.LEVEL, c_obj[0], c_obj[1])
                    ...
                    for e_num, e_obj in enumerate(i["geometry"]["coordinates"]):
                        for f_num, f_obj in enumerate(e_obj):
                            for c_num, c_obj in enumerate(f_obj):
                                coords = self.__vectorToCoords(int(x), int(y) + 1, self.LEVEL, c_obj[0], c_obj[1])
                                i["geometry"]["coordinates"][e_num][f_num][c_num] = coords
                            i["geometry"]["coordinates"][e_num][f_num].append(i["geometry"]["coordinates"][e_num][f_num][0])

                elif i["geometry"]["type"] == "LineString":
                    for e_num, e_obj in enumerate(i["geometry"]["coordinates"]):
                        coords = self.__vectorToCoords(int(x), int(y) + 1, self.LEVEL, e_obj[0], e_obj[1])
...
if __name__ == "__main__":
    geo = Geo()
    data = geo.fetch([52.50876180448243, 13.359631043007212], [ 52.530116236589244, 13.426532801586827])
    print(data.keys())
